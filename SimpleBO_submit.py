...
def extract_primary_key(table):
    ...
    q = 'SELECT COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = ' + \
        str2sqlstr('lahman2017raw') + ' and TABLE_NAME = ' + str2sqlstr(table) + \
...
def extract_all_columns(table):
    q = "SELECT COLUMN_NAME FROM information_schema.columns WHERE table_schema='lahman2017raw' AND table_name=" + \
        str2sqlstr(table)
    raw_results = run_q(q, None, True)
    result = []
    for raw in raw_results:
